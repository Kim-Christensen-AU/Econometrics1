...
\title[\scshape Econometrics 1 (Part I)]{\Large Econometric Analysis of Panel Data}
...
\item[] \text{\large \textcolor{brightmaroon}{\bf 4. FE or RE? The Hausman Test}}
\item[] \text{\large \textcolor{brightmaroon}{\bf 5. Cluster-Robust Inference}}
...
\begin{frame}{Panel Data: Data Structure}
\centering
\renewcommand{\arraystretch}{1.35}
\setlength{\tabcolsep}{0.65em}
\resizebox{0.95\textwidth}{!}{%
\begin{tabular}{c|cccc}
 & \multicolumn{4}{c}{\bf Time} \\
{\bf Cross-section} & $t=1$ & $t=2$ & $\cdots$ & $t=T$ \\
\hline
...
\vspace{0.4cm}
\begin{itemize}
\item Each cell contains the outcome and regressors observed for unit $i$ at time $t$.
\item A balanced panel observes all $N$ units in all $T$ periods.
\end{itemize}
\end{frame}
...
\item Note that the error has the form $\varepsilon_{it} = \alpha_i + u_{it}$
\item Pooled OLS estimator $\hat\vbeta$ will be inconsistent if, for instance, $\alpha_i$ is correlated with $\vx_{it}$, i.e.\ $\E[\vx_{it} \varepsilon_{it}] \neq 0$
\item If $\alpha_i$ and $\vx_{it}$ are uncorrelated then the variance of $
\varepsilon_{it}$ can be estimated accordingly as a sum of two unobserved components (but we can do even better - GLS)
\end{itemize}
\end{enumerate}
\end{frame}
...
\bar{y}_i = \alpha_i + \bar\vx_i' \vbeta + \bar{u}_i
$$
...
$$
\vspace*{-\baselineskip}
\item Note: Cannot estimate coefficients on regressors that are constant over time, i.e.\ have $\vx_{it}=\bar\vx_i$
\end{itemize}
...
$$
\item[$\Rightarrow$] The $N$ intercepts are estimated as $\hat\alpha_i = \bar{y}_i - \bar\vx_i' \hat\vbeta_{\text{FE}},i=1,\ldots ,N$
\end{enumerate}
\end{frame}
...
\item Define $\valpha = (\alpha_1,\ldots,\alpha_N )'$
...
$$
\bar{y}_i = \beta_0 + \bar\vx_i' \vbeta + \underbrace{\alpha_i + \bar{u}_i}_{\bar\varepsilon_i}
$$
\vspace*{-\baselineskip}
...
$$
\hspace*{-1.25cm}
y_{it}-y_{i,t-1} = (\vx_{it}-\vx_{i,t-1})'\vbeta+(u_{it}-u_{i,t-1})
\quad \Rightarrow  \quad
...
\begin{frame}{Setup}
\begin{enumerate}
\item[$\Rightarrow$] In the RE model, we explicitly model $\alpha_{i}$ as a random variable (and, hence, include as part of the error term)
